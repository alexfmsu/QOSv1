...
class QuantumSystem:
    ...
    def __init__(self, cavity_chain=None):
        ...
        if self.__cavity_chain is not None:
            self.set_basis()
            self.__hamiltonian = Hamiltonian(self.__base_states)
    ...
    def print_base_states(self):
        self.__base_states.print()
        # for k, v in self.base_states.items():
        # v.info('-v')
    ...
    def set_basis(self):
        ...
        self.__base_states = BaseStates(base_states)
        # self.__base_states.print()
        # print('ss', ss)
        # exit(0)
